-- Migration for AI Settings Table in Supabase
-- Created: 2024-12-20
...
-- Policy: Only admins and ceos can read ai_settings
-- Note: We need to check the users table for role, but since RLS can't directly access users table,
-- we'll create a more permissive policy and check role (admin or ceo) in the application layer
DROP POLICY IF EXISTS "Admins can read ai_settings" ON public.ai_settings;
CREATE POLICY "Admins can read ai_settings"
  ON public.ai_settings
  FOR SELECT
  USING (true); -- Allow all authenticated users, but we'll check role (admin or ceo) in application code

-- Policy: Only admins and ceos can insert ai_settings
DROP POLICY IF EXISTS "Admins can insert ai_settings" ON public.ai_settings;
CREATE POLICY "Admins can insert ai_settings"
  ON public.ai_settings
  FOR INSERT
  WITH CHECK (true); -- Allow all authenticated users, but we'll check role (admin or ceo) in application code

-- Policy: Only admins and ceos can update ai_settings
...
-- Policy: Only admins and ceos can delete ai_settings
...
-- Create RPC function to get all AI settings
-- This function is accessible to service role and authenticated users
...
How can I help you explore BEATHEOS today?$$, 'The system prompt that defines how Covion Intelligence behaves.')
